...
def auto_run_add_companies(df, max_to_add):
    ...
    for _, row in df.head(max_to_add).iterrows():
        ...
        if website.startswith("http"):
            entries.append(website)           # ← Use URL directly
        else:
            entries.append(row["company"])
    
    entries_str = ",".join([e.replace('"', '').replace(',', ' ') for e in entries])
    
    print(f"\n🚀 Enriching {len(entries)} companies (URLs preferred when available)...")
    cmd = f'python add_companies.py --urls "{entries_str}" --segment "specialty chemicals" --city "Pune"'
